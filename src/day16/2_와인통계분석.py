...
'''
    - 각 플롯(차트)에서 독립변수(각 와인속성) 와 종속변수(등급)간의 선행 관계의 강도를 차트로 표시
    - 기울기 , 잔차의 패턴 , 점들의 분포 등을 관찰하여 독립변수가 종속변수에 미치는 영향 확인
        - 잔차란? 회귀분석에서 실제 데이터와 회귀모델 예측한 값과의 차이
        - 예) 아파트 가격을 예측하는 모델을 만들었는데 , 실제 가격은 10억원대 모델이 예측한 가격은 8억이라고 했을 때 잔차는 2억이다.
